...
export default function StartWorkoutModal({ isOpen, onClose, routines, onSelectRoutine, onStartFree }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex flex-col justify-end pointer-events-auto">
      ...
      >
        <div className="w-full flex justify-center py-4 shrink-0 cursor-pointer" onClick={handleClose}>
        ...
        <div className="px-6 pb-6 flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 no-scrollbar pb-6">
            ...
            <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-3">Tus rutinas</h3>
            
            {routines.map(routine => (
              <button
                key={routine.routine_id}
                onClick={() => {
                  onSelectRoutine(routine.routine_id);
                  handleClose();
                }}
              ...
              >
                ...
                <div className="flex flex-col flex-1 min-w-0">
                  <span className="text-white font-bold text-[16px] truncate">{routine.name}</span>
                  <span className="text-zinc-400 text-xs font-medium truncate">{routine.description || 'Sin descripción'}</span>
                </div>
              </button>
            ))}
          </div>
