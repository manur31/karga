...
export default function SessionDetailModal({ session, onClose }) {
  ...
  const { mutateAsync: updateSessionMutate } = useUpdateSession(
    user?.profile_id,
  );
  const { mutateAsync: updateSessionWithSetsMutate } = useUpdateSessionWithSets(
    user?.profile_id,
  );
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={handleOpenEditSessions}
